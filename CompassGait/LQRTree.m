classdef LQRTree < HybridDrakeSystem
  ...
  methods
    function obj=LQRTree(xG,uG,tvlqr,V)
     
      obj=obj@HybridDrakeSystem(tvlqr.getNumInputs(),tvlqr.getNumOutputs());
      obj = obj.setInputFrame(tvlqr.getInputFrame());
      obj = obj.setOutputFrame(tvlqr.getOutputFrame());
            
      typecheck(xG,'Trajectory');
      typecheck(uG,'Trajectory');
      obj.xG = xG;
      obj.uG = uG;
      
      % add a "do nothing" controller (aka zero controller)
      c0=ConstOrPassthroughSystem(zeros(obj.num_y,1),obj.num_u);
      c0=c0.setInputFrame(tvlqr.getInputFrame());
      c0=c0.setOutputFrame(tvlqr.getOutputFrame());
      ...
      % add the tv limit cycle controller
      [obj,obj.tvlqr_mode_num] = addMode(obj,tvlqr);

      % switch to object coordinates
      V = V.inFrame(obj.getInputFrame);
      
      % switch from tv limit cycle controller to any controller
      obj = addTransition(obj,obj.tvlqr_mode_num,@(obj,t,~,x)1-eval(V,t,x),@transitionIntoAnyFunnel,true,true);
      ...
      for i=1:length(ts)
        n=length(obj.t0);
        obj.S1(nX*n+(1:nX),nX*n+(1:nX))=V.S.eval(ts(i));
        obj.s2(n+1,nX*n+(1:nX))=V.s1.eval(ts(i))';
        obj.s3(n+1,1)=V.s2.eval(ts(i));
        obj.t0(n+1)=ts(i);
        obj.x0(:,n+1)=xtraj.eval(ts(i));
        obj.m(n+1)=mode_num;
      end
      
      obj.V = vertcat(obj.V(:),{V})
      obj = addTransition(obj,mode_num,@(obj,t,~,x)1-V.eval(t,x),@transitionIntoAnyFunnel,true,false);
    ...
    function [obj,ind]=addTrajectory(obj,xtraj,utraj,tvlqr,Vtv,parentID,parentT)
      ...
      if (nargin<6) parentID=obj.tvlqr_mode_num; end
      typecheck(parentID,'double');
      if (parentID<obj.tvlqr_mode_num || parentID>(obj.tvlqr_mode_num+obj.num_tvlqr_modes)) error('invalid parent ID'); end
...
%         for j=1:length(ts)
%           V(j)=obj.Vtv{i}.polyeval(ts(j),x-obj.modes{obj.tvlqr_mode_num+i}.x0.eval(ts(j)));
%         end
%         [Vm,ind] = min(V);
%         if (Vm<Vmin)
%           Vmin=Vm;
...
%      options.warning=false;
      ...
      if (options.stabilize)
        ...
        if (options.verify)
          [ti,Vf] = tvlqr(p,xG,uG,Q,R);
        else
          ti = tvlqr(p,xG,uG,Q,R);
      ...
      if (options.verify)
        ...
        if (isa(p,'PendulumPlant'))
          warning('artificially trimming pendulum tvlqr ROA, since we don''t have saturations implemented yet');
      ...
      while (true)
        ...
        prog = prog.addStateConstraint(FunctionHandleConstraint([0;0;0;0],[0;0;0;0],nX,@(x) c.onTreeConstraint(x),1),nbreaks);
        ...
        if (parent_mode_num==c.tvlqr_mode_num)
          Vparent=Vf;
        else
          Vparent=SpotPolynomialLyapunovFunction(Vf.getFrame(),obj.Vtv{parent_mode_num-c.tvlqr_mode_num}.inFrame(Vf.getFrame).getPoly(parent_T));
        end
        ...
        if (options.verify)
          psys = taylorApprox(feedback(p_nolim,tv),xtraj,utraj,3);
          try 
